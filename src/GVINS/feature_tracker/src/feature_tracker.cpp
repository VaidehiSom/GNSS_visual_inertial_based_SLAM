...
{
    if(FISHEYE)
        m_mask = m_fisheye_mask.clone();
    else
        m_mask = cv::Mat(ROW, COL, CV_8UC1, cv::Scalar(255));
    ...
    for (auto &it : cnt_pts_id)
    {
        if (m_mask.at<uchar>(it.second.first) == 255)
        {
            ...
            cv::circle(m_mask, it.second.first, MIN_DIST, 0, -1);
        }
    }
}
...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time) // feature tracker node
{
    ...
    if (PUB_THIS_FRAME)
    {
        ...
        if (n_max_cnt > 0)
        {
            ...
            if (m_mask.size() != m_forw_img.size())
                cout << "wrong size " << endl;
            cv::goodFeaturesToTrack(m_forw_img, m_n_pts, MAX_CNT - m_forw_pts.size(), 0.01, MIN_DIST, m_mask); //! check the function
        }
        else
            m_n_pts.clear();
...
void FeatureTracker::rejectWithF()
{
    if (m_forw_pts.size() >= 8)
    {
        ...
        // reduce vectors based on status
        reduceVector(m_prev_pts, status);
        reduceVector(m_cur_pts, status);
        reduceVector(m_forw_pts, status);
        reduceVector(m_cur_un_pts, status);
